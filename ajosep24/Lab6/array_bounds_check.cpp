...
// It continues to output even when it reaches the end of the array
//   Why is no error raised?
// Because the compiler doesn't check for that because the loop is just continuing
// through the heap after the array ends.
//   How many bytes are between the start of str1 and str2?
// 28
//   How could you test for this logical error?
// Check to see if i < new char size, during the loop
//   How could you prevent this type of error?
// Ensure any loops are < the size of the array the the pointer is pointing to
//
//   Include the delete[] at then end and recompile, what changes?
// It gives a message saying free(): invalid size, but the program still runs
